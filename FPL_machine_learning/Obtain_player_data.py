...
pd_in = pd.read_csv("players_raw.csv")
...
players_dir = '../FPL_Machine_learning/Fantasy-Premier-League-master/data/2019-20/players'
    ...
    for file in files:
        if file == "gw.csv":
            ...
            if len(data["round"]) > 12:
                ...
                # From training data
                if chance_playing_next_round != str(1000):
                    points, n_points, acc = predicted_points(team_code, data, training_counts, n)
                    print("player {} has been trained".format(web_name))
